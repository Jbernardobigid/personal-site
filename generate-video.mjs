...
// it; v2 top-right -> pan-up lands on it; etc.). Photo b-roll cycles the same
// motions so stills never sit frozen behind moving text.
const DESIGN_MOTIONS = ['zoom-in', 'pan-up', 'pan-down', 'zoom-out', 'zoom-in-slow'];

// zoompan MUST consume a single input frame with d=N (all frames generated by the
// filter): on a looped image stream with d=1, its `zoom` variable resets to 1.0 on
// every input frame, so recursive z expressions never accumulate — verified on
// ffmpeg 8.1: every "moving" scene rendered as a static frame. All motion is
// therefore expressed as a function of scene progress (on/N), never of prior zoom.
function motionFilter(kind, duration) {
  const N = Math.max(1, Math.round(duration * FPS));
  const cx = `x='iw/2-(iw/zoom/2)'`, cy = `y='ih/2-(ih/zoom/2)'`;
  const tail = `d=${N}:s=${W}x${H}:fps=${FPS}`;
  const p = `(on/${N})`;
  const range = +(BG_MAX_ZOOM - 1).toFixed(3);
  switch (kind) {
    case 'pan-up': return `zoompan=z=${BG_MAX_ZOOM}:${cx}:y='(ih-ih/zoom)*(1-${p})':${tail}`;
    case 'pan-down': return `zoompan=z=${BG_MAX_ZOOM}:${cx}:y='(ih-ih/zoom)*${p}':${tail}`;
    case 'zoom-out': return `zoompan=z='${BG_MAX_ZOOM}-${range}*${p}':${cx}:${cy}:${tail}`;
    case 'zoom-in-slow': return `zoompan=z='1+${+(range * 0.6).toFixed(3)}*${p}':${cx}:${cy}:${tail}`;
    default: return `zoompan=z='1+${range}*${p}':${cx}:${cy}:${tail}`;
  }
}

function buildSceneClip({ intro, headline, audioPath, duration, broll, fadeOut, outPath }) {
  const d = duration.toFixed(3);
  const args = ['-y'];
  if (broll?.type === 'video') args.push('-stream_loop', '-1', '-t', d, '-i', broll.path);
  // Stills go in as ONE frame — zoompan in motionFilter generates the whole scene
  // from it (see the reset note above motionFilter).
  else if (broll?.type === 'photo' || broll?.type === 'design') args.push('-i', broll.path);
  ...
  if (broll?.type === 'video') {
    ...
    // Photo is scaled 1.2x oversize (like the design canvas) so the Ken Burns
    // move always samples real pixels, then gets the same per-scene camera moves.
    bg = `[0:v]scale=${BG_W}:${BG_H}:force_original_aspect_ratio=increase,crop=${BG_W}:${BG_H},${motionFilter(broll.motion, duration)},eq=brightness=-0.24:saturation=0.95,vignette=PI/4.2[bg]`;
    ...
    bg = `[0:v]${motionFilter(broll.motion, duration)},noise=alls=5:allf=t,vignette=PI/5[bg]`;
  } else {
    bg = `[0:v]vignette=PI/4.5[bg]`;
  }
  // No fade-in from black: Instagram uses frame 0 as the feed thumbnail, so the
  // video must open on the composed background (the text layers still fade in).
...
async function main() {
  ...
  const clips = [];
  for (let i = 0; i < script.scenes.length; i++) {
    if (brolls[i]?.type === 'photo') brolls[i].motion = DESIGN_MOTIONS[i % DESIGN_MOTIONS.length];
    ...
    buildSceneClip({
      ...
      fadeOut: i === script.scenes.length - 1, outPath: clip
    });
    clips.push(clip);
  }
