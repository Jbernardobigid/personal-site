...
- `social/` is the IG media staging area (must be committed/pushed so Vercel serves public URLs). Folders are deletable after the post is live — `npm run social:recycle` shows the plan, `social:recycle:apply` deletes and commits. It only removes folders whose Notion card is Published/Archived (+7d grace); no-card folders wait 30d and a Notion failure aborts. The VPS runs it weekly (workflow `XmgX6V4fGMac4akU`). Left unpruned this directory is what put the Vercel free team over its 10 GB Deployment Storage cap. Store nothing else there.
