...
async function main() {
  ...
  // Validate imagePath stays within the expected images directory to prevent
  // an attacker-tampered post-meta.json from reading arbitrary files.
  const IMAGES_DIR = path.resolve(__dirname, 'blog', 'posts', 'images');
  if (imagePath) {
    const resolvedImage = path.resolve(imagePath);
    if (!resolvedImage.startsWith(IMAGES_DIR + path.sep)) {
